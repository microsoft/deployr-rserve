...
int wfork(int socket, char* parentCmdLine, int idx)
{
  ...
  m = WaitForSingleObject(ghMutex, INFINITE);
  if (m == WAIT_OBJECT_0) {
	  winSocks[idx] = socket;
...
#ifdef WIN32
DWORD WINAPI cancelValidateConn(int *i) {
        int sfd = *i;
        free(i);
#else
void *cancelValidateConn(void *ifd) {
        int sfd = (int) ifd;
#endif
        int rn = 0;
        int clientPid = 0;
	...
	send(sfd, (char*) IDstring, 32, 0);


	rn = recv(sfd, (char*)&recvBuff, 10, 0);
        recvBuff[rn] = 0;
        cmd = atoi (recvBuff);
...
//    fd_set readfds;
   ...
   // If iMode != 0, non-blocking mode is enabled.
      int iMode = 0;
      ioctlsocket(ss, FIONBIO, (u_long FAR*) &iMode);
      ioctlsocket(cs, FIONBIO, (u_long FAR*) &iMode);
#endif	
...
#ifdef Win32
	HANDLE thread = CreateThread(NULL, 0, cleanUpLoop, NULL, 0, NULL);
#endif
...
#else
        selRet = select(maxfd + 1, &readfds, 0, 0, NULL);
#endif
